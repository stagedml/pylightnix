...
from pylightnix.groups import (Key, store_config, Tag, RRefGroup, groupmain,
                               tag_out, groups2paths)
...
class BuildError(Exception):
  ...
  def __init__(self,
    ...
    self.outgroups:List[Dict[Path,Tag]]=outgroups
  def __str__(self):
...
def build_paths(b:Build, refpath:RefPath, tag:Tag=Tag('out'))->List[Path]:
  ...
  if refpath[0]==b.dref:
    ...
    return [Path(join(rref2path(rg[tag],b.storage), *refpath[1:]))
...
def repl_continueBuild(b:Build, rh:Optional[ReplHelper]=None)->Optional[RRef]:
  return repl_continue(out_paths=groups2paths(b.outgroups), rh=rh)
